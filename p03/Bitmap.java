...
// Classe representando o contedo de um arquivo bitmap
public class Bitmap {
	// campos do cabealho bitmap
    ...
    public int[][][] raster;
    // RGB, onde raster[i][j]  a posio do pixel (i,j)
    // raster[i][j][0]  o componente R em (i,j)
    // raster[i][j][1]  o componente G em (i,j)
		// raster[i][j][2]  o componente B em (i,j)

    //  o array de bytes com dados do arquivo BitMap em memria, que ser salvo
    byte[] bitmapfile;
    ...
    Bitmap(String filename) throws Exception{
        ...
        // Leitura dos campos do cabealho BMP
    ...
    public void cuiftoBitmap(String filename) throws IOException {
    	...
    	int version = cuiffile[2];
   	 	...
   	 	size_of_bitmap_file = 54 + height*width*3; // Cabealho de 54 bytes
   	 	int i=0;
   	 	...
   	 	offset_to_start_image = 54; // 54 bytes usado no cabealho
   	 	...
   	 	if (version==1)
   	 		...
   	 		throw new IOException("Verso no suportada");

    }

   	// Mtodo que l os pixels do arquivo Cuif1 e gera raster, incluindo no array de bytes do arquivo
    ...
    // Mtodo para salvar o arquivo em "filename"
